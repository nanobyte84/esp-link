...
ESPPORT		?= /dev/tty.usbserial-AI02KGRE
ESPBAUD		?= 460800
...
# Optional Modules
#MODULES ?= user-libraries/Adafruit_GFX user-libraries/Adafruit_SSD1306 driver driver/i2c mqtt rest syslog 
MODULES ?= user-libraries/SSD1306 driver driver/i2c mqtt rest syslog 
...
C_OBJ		:= $(patsubst %.c,$(BUILD_BASE)/%.o,$(C_SRC)) $(BUILD_BASE)/espfs_img.o
CXX_OBJ		:= $(patsubst %.cpp,$(BUILD_BASE)/%.o,$(CXX_SRC))  $(BUILD_BASE)/espfs_img.o
OBJ		    :=  $(C_OBJ) $(CXX_OBJ)
...
CXXFLAGS	= -Os -g -Wpointer-arith -Wundef -Werror -Wl,-EL -nostdlib -mlongcalls -mtext-section-literals -finline-functions -fdata-sections -ffunction-sections -D__ets__ -DICACHE_FLASH -D_STDINT_H -DARDUINO=106 -fno-rtti -fno-exceptions -std=c++11 -felide-constructors
...
build/eagle.esphttpd1.v6.ld: $(SDK_LDDIR)/eagle.app.v6.new.1024.app1.ld
	$(Q) sed -e '/\.irom\.text/{' -e 'a . = ALIGN (4);' -e 'a *(.espfs)' -e '}'  \
			-e '/^  irom0_0_seg/ s/6B000/7C000/' 			$(SDK_LDDIR)/eagle.app.v6.new.1024.app1.ld >$@
